const getNewsFromNewTalks = async () => {
  try {
    ...
    const fetch2 = fetch("https://forumapi.newtalk.tw/api/Comment/QueryNewList",
      {
        method: "POST",
        headers: {
          'Content-Type': 'application/x-www-form-urlencoded;charset=UTF-8'
    ...
    const posts = res.content.articleList.concat(res2.content.commentDetailList).map((item) => `${item.articleTitle}\n${item.metaInfo.url}`);
    return posts.join("\n");
  }
  catch (err) {
    throw Error(err);
